...
class Game:
    ...
    def startScreen(self):
        ...
        play = Button((W/2, 2*H/3), 80, 'Jouer',None,GREEN)
        settings = Button((150, H-75), 50, 'Paramétres',None,BLUE)
        rules = Button((W-100, H-75), 50, 'Régles',None, RED)
        ...
        while start:
            ...
            textOut(self.screen, "JEU D'HEX", 100, WHITE, (W/2, H/3))
            # show buttons
            for button in buttons:
    ...
    def rulesScreen(self):
        ...
        while start:
            ...
            # --------------------STUFF-----------------------
            self.screen.blit(self.bg_img, (0, 0))
            textOut(self.screen, 'Régles', 100, RED, (W/2, H/3))
            textOutMultiline(self.screen, self.rules_text, 20, WHITE, (W/2, H/3))
            # show buttons
            for button in buttons:
                button.show(self.screen)
            # double processing
    ...
    def settingsScreen(self):
        ...
        while start:
            ...
            # --------------------STUFF-----------------------
            self.screen.blit(self.bg_img, (0, 0))
            textOut(self.screen, 'Paramétrages', 80, WHITE, (W/2, H/4))
            textOut(self.screen, 'Taille du jeu:', 50, WHITE, (W/3, H/2))
            textOut(self.screen, self.size, 50, WHITE, (2*W/3, H/2))
            textOut(self.screen, 'Musique:', 50, WHITE, (W/3, H/2+60))
            textOut(self.screen, 'Son:', 50, WHITE, (W/3, H/2+120))
    ...
    def pauseScreen(self):
        ...
        start = True
        # initializing buttons
        resume = Button((W/2, H/3), 80, 'Continuer', col=BLACK)
        home = Button((W/2, H/2), 50, 'Menu', col=BLACK)
        buttons = [home, resume]
        while start:
            # sticking to fps
            self.clock.tick(FPS)
    ...
    def GOScreen(self, winner):
        ...
        while go:
            ...
            self.showGrid()
            self.shadow()
            textOut(self.screen, 'TERMINER', 80, RED, (W/2, H/3))
            if winner == 2:
                textOut(self.screen, 'Le joueur bleu gagne la partie', 30, BLACK, (W/2, H/2))
            else:
                textOut(self.screen, 'Le joueur vert gagne la partie ', 30, BLACK, (W/2, H/2))
